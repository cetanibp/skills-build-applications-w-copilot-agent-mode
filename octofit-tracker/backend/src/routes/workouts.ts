...
router.get('/', async (_req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Unable to fetch workouts.', details: (error as Error).message });
  }
});
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    return res.status(400).json({ error: 'Unable to create workout.', details: (error as Error).message });
  }
});
